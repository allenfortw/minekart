  ...
  implements Listener
{
  ...
  private boolean AttemptShopOpen(Player player, LivingEntity entity) {
    if ((!this._openedShop.contains(player.getName())) && (entity.isCustomNameVisible()) && (entity.getCustomName() != null) && (ChatColor.stripColor(entity.getCustomName()).equalsIgnoreCase(ChatColor.stripColor(this.Name))))
    {
  ...
  {
    if ((this.PlayerPageMap.containsKey(event.getPlayer().getName())) && (((ShopPageBase)this.PlayerPageMap.get(event.getPlayer().getName())).getTitle().equalsIgnoreCase(event.getInventory().getTitle())))
    {
